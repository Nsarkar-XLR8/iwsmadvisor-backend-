...
const caseStudyPayloadFromRequest = (req) => ({
  title: pickField(req.body, ['title', 'Title', 'title ', 'Title ']),
  description: pickField(req.body, [
    'description',
    'Description',
    'description ',
    'Description '
  ...
  challenge: pickField(req.body, ['challenge', 'Challenge']),
  solution: pickField(req.body, ['solution', 'Solution']),
  benefit: pickField(req.body, ['benefit', 'Benefit']),
  customer: pickField(req.body, ['customer', 'Customer']),
  client: pickField(req.body, ['client', 'Client']),
  duration: pickField(req.body, ['duration', 'Duration']),
  teamSize: pickField(req.body, [
    'teamSize',
    'team_size',
    'TeamSize',
    'Team Size'
  ]),
  technologiesUsed: pickField(req.body, [
    'technologiesUsed',
    'technologies',
    'TechnologiesUsed',
    'Technologies'
  ]),
  resultImpact: pickField(req.body, [
    'resultImpact',
    'result',
    'ResultImpact',
    'Result'
  ]),
  caseExperience: pickField(req.body, [
    'caseExperience',
    'experience',
    'CaseExperience',
    'Experience'
  ]),
  clientName: pickField(req.body, ['clientName', 'client_name', 'ClientName']),
  companyName: pickField(req.body, [
    'companyName',
    'company_name',
    'CompanyName'
  ])
...
export const createCaseStudy = async (req, res) => {
  try {
    ...
    const caseStudy = await createCaseStudyService({
      ...caseStudyPayloadFromRequest(req),
      image: imageFile
    });
...
export const getCaseStudyById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ success: false, message: 'Invalid case study id' });
    }
...
export const updateCaseStudy = async (req, res) => {
  try {
    ...
    const imageFile =
      ...
      req.files?.image?.[0] ||
      firstAvailableFile(req.files);
    const payload = removeUndefined(caseStudyPayloadFromRequest(req));
...
export const deleteCaseStudy = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: 'Case study deleted successfully' });
  } catch (error) {
