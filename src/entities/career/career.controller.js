...
export const getCareers = async (req, res) => {
  try {
    const { page, limit, search, type, isActive, multiplePosition } = req.query;
...
export const getCareerById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ success: false, message: 'Invalid career id' });
    }
...
export const deleteCareer = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: 'Career deleted successfully' });
  } catch (error) {
